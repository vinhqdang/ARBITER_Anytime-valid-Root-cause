...
better than chance). Over a deployment horizon of $T$ rounds, the harm an
optimally evasive strategic agent extracts before an anytime-valid e-process
run at level $\alpha$ detects it satisfies
$$B(p,\varepsilon,T) \;=\; \frac{\sqrt{2\pi_0(1-\pi_0)\log(1/\alpha)\,T}}{\kappa\,(1-\varepsilon)\,\sqrt{p}}\;\big(1+o(1)\big)$$
as $\varepsilon \to 1$ or $T \to \infty$ with the other parameters fixed.
\end{theorem}

\begin{proof}[Proof sketch]
Write $\delta := \mathrm{flag}(q) - \pi_0 = \kappa(1-\varepsilon)q$. The
second-order expansion of the binary KL divergence gives, for small
$\delta$, $\mathrm{KL}(\pi_0+\delta \,\|\, \pi_0) \approx \delta^2/(2\pi_0(1-\pi_0))$.
Detection requires $\log(1/\alpha)/\mathrm{KL}$ audited samples, arriving at
rate $p$ per round, so rounds-to-detection is $\mathrm{rounds}(q) \approx
K/q^2$ with $K := 2\pi_0(1-\pi_0)\log(1/\alpha)/(\kappa^2(1-\varepsilon)^2p)$.
Harm is $h(q) = q\cdot\min(K/q^2, T)$: increasing in $q$ below the crossover
$q^* = \sqrt{K/T}$, decreasing above it, so the adversary's optimum is at
$q^*$, giving $B = h(q^*) = \sqrt{KT}$, which is the claimed expression. The
$o(1)$ term absorbs the small-$\delta$ approximation error, which vanishes
as the optimal $\delta^* = \kappa(1-\varepsilon)q^*$ itself shrinks---exactly
the regime an evasive adversary operates in by construction. \qed
\end{proof}

We confirmed both the exponents and the constant of Theorem~\ref{thm:scaling}
numerically: fitting $\log B$ against $\log T$, $\log(1-\varepsilon)^{-1}$,
and $\log p^{-1}$ independently across three parameter sweeps recovers
exponents $0.4876$, $1.0016$, and $0.5049$ against the predicted $0.5$,
$1$, $0.5$ (max residual $0.02$ in log-space); the closed-form constant
matches a direct grid-search optimum of $h(q)$ to within $2\%$ across five
independent $(p,\varepsilon,T)$ points (Section~\ref{sec:experiments}).
...
auditing dominates any targeted alternative.} A closed-form expression for
$w^*$ follows the same small-deviation expansion as
Theorem~\ref{thm:scaling} applied to both harm terms simultaneously; we
leave its derivation to an extended version and report the threshold
numerically here.
...
\textbf{Theorem~\ref{thm:scaling} is asymptotic in the small-deviation
regime.} The KL approximation underlying it is self-consistent in exactly
the regime an evasive adversary operates in (low audited defection rate by
construction), which our numerical checks confirm, but a non-asymptotic
version with explicit error bounds is left to an extended version.

\textbf{We do not give a closed form for the operating threshold $w^*$.}
Section~\ref{sec:algorithm}'s threshold is numerically established across a
range of parameters but not yet derived in closed form; the same method that
produced Theorem~\ref{thm:scaling} should apply, applied jointly to the
strategic and naive harm terms.
